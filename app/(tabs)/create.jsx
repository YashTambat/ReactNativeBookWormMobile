...
export default function Create() {
  ...
  const pickImage = async()=>{
    try {
      ...
      //launched the image library
      const result = await  ImagePicker.launchImageLibraryAsync({
        mediaTypes:'images',
        allowsEditing:true,
         aspect:[4,3],
         quality:0.5, // Allow the quality for smaller base64
         base64:true,
      })
      if (!result.canceled)
      {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Always assume you are uploading PNG/JPEG depending on your app
      let mimeType = "image/jpeg"; // default
      if (image && image.toLowerCase().endsWith(".png")) {
        mimeType = "image/png";
      }
  ...
  const renderRatingPicker = () => {
    ...
    return <View style={styles.ratingContainer}>{stars}</View>; // You can wrap it in a View if needed
  };
